...
class VoxelMap extends Element {
    ...
    async destroy() {
        ...
        for (let class_id in this.classes) {
            this.field.scene.remove(this.classes[class_id].mesh);
        }
    }

    hide() {
        for (let class_id in this.classes) {
            this.classes[class_id].mesh.visible = false;
        }
    }

    show() {
        for (let class_id in this.classes) {
            this.classes[class_id].mesh.visible = true;
        }
    }
    ...
    {
        for (let class_id in this.classes) {
            this.classes[class_id].mesh.position.set(this.datastate.x, this.datastate.y, this.datastate.z)

            // Datastate is a pose
            if (this.datastate.yaw != undefined)
            {
                this.classes[class_id].mesh.rotation.setFromVector3(
