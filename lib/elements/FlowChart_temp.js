...
class FlowChart extends Element {
    ...
    setup() {
        ...
        this.link = this.field.plot.selectAll('.link')
        .append('g')
        ...
        this.node = this.field.plot.selectAll('.node')
        .attr("id", this.id)
        ...
        var labels = this.node.append("text")
        .attr("dx", 10)
        .attr("dy", 0)
        ...
        this.simulation.on('tick', () => {
          ...
          attrsFunction(this.link, (link)=>this.calculate_link_coords(link))
    ...
    draw() {  
          ...
          // Bind data to nodes and create node elements
      this.node = this.field.plot.selectAll('.node')
      .data(this.datastate.nodes, function(d) { return d.id; });
      ...
      let labels = nodeEnter.append("text")
      .attr("dx", 10)
      .attr("dy", 0)
      .text(function(d) { return d.title });
      ...
      // Update the links
      this.link = this.field.plot.selectAll('.link')
      .data(this.datastate.links, function(d) { return d.source + '-' + d.target; });

      // Enter any new links
      let linkEnter = this.link.enter().append('line')
      .attr('class', 'link')
      .attr('stroke-width', function(d){ return 1 })
      .attr("stroke", "black")
      .attr("marker-end", "url(#arrowhead)");
      ...
      this.link_label = this.field.plot.selectAll('.link_label')
      .data(this.datastate.links, function(d) { return d.source + '-' + d.target; });

      // Enter any new link labels
      let link_labelEnter = this.link_label.enter().append('text')
      .attr("text-anchor", "middle")
      .attr("paint-order", "stroke")
      .attr("stroke", "#ffffff")
      .attr("stroke-width", "5px")
