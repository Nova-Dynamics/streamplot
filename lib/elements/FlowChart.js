...
class FlowChart extends Element {
    ...
    draw() {  
      ...
      //   this.class_id = `flowchart-${this.id}`

      //   //this.field.plot.selectAll(`.${this.class_id}`).remove();
        
      // console.log("BEFORE: ", this.datastate.nodes)

      // const old = new Map(this.node.data().map(d => [d.index, d]));
      // this.datastate.nodes = this.datastate.nodes.map(d => Object.assign(old.get(d.index) || {}, d));
      // this.datastate.links = this.datastate.links.map(d => Object.assign({}, d));

      // console.log("AFTER: ", this.datastate.nodes)

      //this.simulation.nodes(this.datastate.nodes);
      //this.simulation.force("link").links(this.datastate.links);
      //this.simulation.alpha(1).restart();
      this.field.plot.selectAll(`.${this.class_id}`)
      .data(this.datastate.nodes)
