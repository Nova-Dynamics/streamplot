...
class Vis3D extends Field {
    ...
    setup() {
        ...
        // Create the html side of the renderer if one wasn't provided
        ...
        if (this.auto_render)
            this.render(); // Also starts the render loop
    ...
    static MapboxLayer(id, config, mercator_coordinates, rotation) {
        // Hack in a fake window object because we will be using an external canvas
        let sp_window = { add_field: ((x) => "vis3d_id") };
    
        let layer = {
            id,
            type: 'custom',
            renderingMode: '3d',
            axis: null,  // Initialize axis as a property of layer
            onAdd: function (map, gl) {
    
                this.axis = new Vis3D(sp_window, {}, {
                    auto_render: false,
                    canvas: map.getCanvas(),
                    context: gl,
                    use_mouse_view: false,
                    camera: new THREE.Camera(),
                    set_default_camera_position: false,
                    ...config
                });
    
                this.axis.setup();
    
                this.axis.camera.position.z = 0;
                this.axis.camera.position.y = 0;
                this.axis.camera.position.x = 0;

                // Set camera rotation to zero
                this.axis.camera.rotation.x = 0;
                this.axis.camera.rotation.y = 0;
                this.axis.camera.rotation.z = 0;
    
                this.axis.renderer.autoClear = false;
    
                this.axis.mapbox_scene_transform = {
                    translation: [mercator_coordinates.x, mercator_coordinates.y, mercator_coordinates.z],
                    rotation,
                    scale: mercator_coordinates.meterInMercatorCoordinateUnits()
                }

                this.axis.mapbox_render_enabled = () => true;
    
                this.map = map;
            },
            render: function (gl, matrix) {

                if (!this.axis.mapbox_render_enabled()) return;

                let rotationX = new THREE.Matrix4().makeRotationAxis(
                    new THREE.Vector3(1, 0, 0),
                    this.axis.mapbox_scene_transform.rotation[1]
                );
                let rotationY = new THREE.Matrix4().makeRotationAxis(
                    new THREE.Vector3(0, 1, 0),
                    this.axis.mapbox_scene_transform.rotation[2]
                );
                let rotationZ = new THREE.Matrix4().makeRotationAxis(
                    new THREE.Vector3(0, 0, 1),
                    -this.axis.mapbox_scene_transform.rotation[0]
                );
                ...
                let l = new THREE.Matrix4()
                    .makeTranslation(
                        this.axis.mapbox_scene_transform.translation[0],
                        this.axis.mapbox_scene_transform.translation[1],
                        this.axis.mapbox_scene_transform.translation[2]
                    )
                    .scale(
                        new THREE.Vector3(
                            this.axis.mapbox_scene_transform.scale,
                            -this.axis.mapbox_scene_transform.scale,
                            this.axis.mapbox_scene_transform.scale
                        )
                    )
                ...
                this.axis.camera.projectionMatrix = m.multiply(l);
    
                this.axis.renderer.resetState();
                this.axis.render();
